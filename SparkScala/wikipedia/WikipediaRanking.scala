...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setAppName("wikipedia_ranking").setMaster("local[*]")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)((count, article) => article.mentionsLanguage(lang) match { 
      case true => count + 1 
      case _ => count + 0 
      }, (a, b) => a + b)
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(article => langs.map(lang => (lang, article.mentionsLanguage(lang) match { 
      case true => 1 
      case false => 0 
    }))).reduceByKey((x, y) => x + y)
        .collect()
