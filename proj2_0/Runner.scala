...
object Runner {
  ...
  def helloTweetStream(spark: SparkSession): Unit = {
    ...
    // by default hits the sampled stream and uses "twitterstream" 
    // as the directory We'll run it in the background using a Future:
    // We're not saving a reference to this future or providing a 
    // callback function we just start it running in the background 
    // and forget about it.
    ...
    //Here we're just going to wait until a file appears in our 
    // twitterstream directory or until some reasonable amount of 
    // time has passed (30s)
    var start = System.currentTimeMillis()
    var filesFoundInDir = false
    while(!filesFoundInDir && (System.currentTimeMillis()-start) < 30000000) {
    ...
    // this will fail if you haven't already 
    // streamed down some twitter data.
    ...
    // Display placenames as tweets occur there
    // Have to deal with a good chunk of nested data
    // Writing a case class and using DataSets would 
    // be more initial investment, but would make
    // writing queries like this much easier!
    // streamDf
    ...
    // streamDf
    //   .select($"data.text", $"data.lang", $"data.geo.place_id")
    //   .writeStream
    //   .outputMode("append")
    //   .format("console")
    //   .start()
  ...
  def helloSparkSql(spark: SparkSession): Unit = {
    ...
    val demoQuery = df.filter(functions.length($"name.first") < 6).groupBy($"eyeColor").agg(functions.avg($"age"))
    ...
    val demoQuery2 = ds.filter(_.age > 30).map((person) => { s"${person.name.first} ${person.name.last}" })
      .toDF(
        "Full Name"
